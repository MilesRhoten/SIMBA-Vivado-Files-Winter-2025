...
module cntr_udclr_nb  #(parameter n=8) (clk, clr, up, dn, en, ld, D, count, rco); 
    input  clk, clr, up, dn, en, ld; 
    input  [n-1:0] D; 
    ...
    begin 
        if (clr == 1)       // asynch reset
           ...
           count <= D; 
            ...
            begin
                if (up == 1)   // count up (increment)
                    count <= count + 1; 
                else if (dn == 1)   // count down (decrement)
                    count <= count - 1; 
           end 
    end 
